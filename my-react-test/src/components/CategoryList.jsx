...
const CategoryList = ({ moduleId, onSelectCategory }) => {
    ...
    const filterCategory = (text) => {
        ...
        const filteredParents = topLevel.filter(
            (p) =>
                ...
                (childrenMap[p.title] &&
                    childrenMap[p.title].some((c) => c.title.toLowerCase().includes(lower)))
        );
    ...
    const { topLevel: filteredParents, childrenMap: filteredChildren } = filterCategory(search);

    return (
        <aside className="w-[22%] border-r bg-gray-50 p-3 overflow-y-auto h-full">
            {/* Search box */}
            <input
                type="text"
